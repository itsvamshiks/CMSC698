...
def load_data(path):
    data = [l.strip() for l in open(path)]
...
def demo(seed=123):
    ...
    try:
        ...
        sys.exit(1)
    """
    opendata = [l.strip() for l in open('iris.data') if l.strip()]
    features = [tuple(map(float, x.split(',')[:-1])) for x in opendata]
    labels = [x.split(',')[-1] for x in opendata]
    data = dict(zip(features, labels))
    features = data.keys()
    random.seed(seed)
